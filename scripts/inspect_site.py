#!/usr/bin/env python3
"""Inspect the deployed portfolio page and report what a picky visitor would notice.

Checks the live URL, not the working copy, because a page that only passes
locally has not been tested. Reports; it never edits prose. Findings are
written to findings.md for the workflow to raise as a single issue.
"""
import io, json, os, re, sys, urllib.error, urllib.request

SITE = os.environ.get('SITE', 'https://jeevan-0508.github.io/')
...
def fetch(url, method='GET'):
    req = urllib.request.Request(url, method=method, headers={
        'User-Agent': 'jeevan-0508-portfolio-inspector',
        'Accept': '*/*',
    })
...
def main():
    ...
    if len(re.findall(r'<h1[ >]', html)) != 1:
        note('major', 'one h1', 'the page has %d <h1> elements' % len(re.findall(r'<h1[ >]', html)))
    ...
    # --- contact route: the thing that costs opportunities ---
    if not re.search(r'href="mailto:', html):
        note('major', 'contact route', 'no email address anywhere on the page')
    if 'linkedin.com/in/' not in html:
        note('major', 'contact route', 'no LinkedIn profile link')

    # --- weight ---
    assets = set(re.findall(r'(?:href|src)="([^"]+\.(?:css|js|png|jpg|jpeg|svg|webp))"', html))
    for rel in sorted(assets):
        url = rel if rel.startswith('http') else SITE.rstrip('/') + '/' + rel.lstrip('/')
        try:
            _, blob, _ = fetch(url)
        except Exception as e:
    ...
    # --- the repositories the page sends people to ---
